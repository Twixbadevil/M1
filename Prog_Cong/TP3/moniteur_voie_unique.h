...
typedef struct moniteur_voie_unique_s
{
  ...
  //variable conditionnelle pour attente passive d'acces a la voie
  pthread_cond_t condVide;
  pthread_cond_t condPleine;
  //variable conditionnelle pour sens de circulation de la voie
  pthread_cond_t condEst;
  pthread_cond_t condOuest;
  
  //mutex controlant l'acces au moniteur
  pthread_mutex_t mutexMonit;

  /* Attributs */
  voie_unique_t * voie_unique ;		/* section de a voie unique */
  int cpt; //nombre de train empruntant la voie
  int max; //nombre maximal de train pouvant emprunter la voie
  sens_t s;
