...
pthread_mutex_t mutexCarte[4] = {PTHREAD_MUTEX_INITIALIZER,PTHREAD_MUTEX_INITIALIZER,PTHREAD_MUTEX_INITIALIZER,PTHREAD_MUTEX_INITIALIZER};
...
void fonc_joueur(int i){
  ...
  while( ! fini ) 
    ...
    echange=FAUX ; 
		  ...
		  pthread_mutex_lock(&(mutexCarte[ind_carte]));
...
main( int argc , char * argv[] ) 
{
 ...
 if( argc != 2 ) 
  ...
  for(i=0 ; i<NbJoueurs ; i++ ) 
   ...
   for(i = 0; i < NbJoueurs ; i++){
   	pthread_create(&thread[i],NULL,(void *)fonc_joueur,(int *)i);
   }
